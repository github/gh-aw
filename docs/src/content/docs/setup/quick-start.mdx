...
`repo-status` gathers recent repository activity (issues, PRs, discussions, releases, code changes) and opens a GitHub issue summarizing productivity insights, community highlights, and recommended next steps, as described in its frontmatter:
...
See the full source at [`workflows/repo-status.md`](https://github.com/githubnext/agentics/blob/main/workflows/repo-status.md?plain=1).
...
To avoid a long-lived key, configure [Anthropic Workload Identity Federation](/gh-aw/reference/auth/#anthropic-workload-identity-federation-wif).
