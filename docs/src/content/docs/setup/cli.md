...
**Options:** `--after-run-id`, `--artifacts`, `--before-run-id`, `--cache-before`, `--cached-logs`, `--cached-json`, `--count/-c`, `--end-date`, `--engine/-e`, `--evals`, `--exclude-staged`, `--filtered-integrity`, `--firewall`, `--format`, `--json/-j`, `--last`, `--no-firewall`, `--output/-o`, `--parse`, `--ref`, `--report-file`, `--repo/-r`, `--runtime`, `--safe-output`, `--start-date`, `--stdin`, `--summary-file`, `--timeout`, `--tool-graph`, `--train`

`logs` defaults `--artifacts` to `usage` for faster, compact downloads. The `--last` flag is an alias for `--count/-c`.

`--cached-logs` reuses compatible runs from an earlier logs JSON response and periodically overwrites that file with current results. `--cached-json` is an alias.
