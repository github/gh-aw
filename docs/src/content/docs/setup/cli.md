...
from standard input. Operational-value evaluators receive the standard run,
event, and config request and print their ordered named metrics.

```bash wrap
gh aw graders run weekly-research loops 123456789
cat payload.json | gh aw graders run weekly-research loops
cat payload.json | gh aw graders run daily-file-diet operational-value
```
