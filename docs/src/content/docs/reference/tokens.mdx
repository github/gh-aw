...
If using the Claude by Anthropic engine, you need to configure `ANTHROPIC_API_KEY` with an API key from Anthropic.
...
If using the Codex by OpenAI engine, you need to configure `OPENAI_API_KEY` with an API key from OpenAI.
...
GitHub Actions automatically provides this token with scoped access to the current repository. It is used by default to checkout the repository and for accessing GitHub via the GitHub MCP server running in local mode.
...
- Capabilities: read access to the current repository; permissions follow workflow `permissions:`.
- Limitations: no cross-repo access, cannot trigger workflows via API, not supported for remote GitHub MCP server mode.

---

### `GH_AW_GITHUB_TOKEN`

If using cross-repo access or the remote GitHub MCP server, you need to configure this GitHub Actions secret a Personal Access Token (PAT) with appropriate scopes and permissions.
...
If you need different permissions for the GitHub MCP server than the default token chain, you can configure `GH_AW_GITHUB_MCP_SERVER_TOKEN` with a Personal Access Token (PAT) or use a GitHub App for enhanced security.

This specialized token for the GitHub MCP server that can override the standard token chain. Use this when you need different permission levels for MCP server operations versus other workflow operations.
