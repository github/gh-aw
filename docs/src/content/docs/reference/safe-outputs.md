...
| [Add Labels](#add-labels-add-labels) | `add-labels` | Add labels to issues or PRs (max: 10 calls; max-added-labels: 10 per call, capped at 100) |
...
safe-outputs:
  add-labels:
    ...
    max: 3                       # max add_labels calls (default: 10)
    max-added-labels: 3          # max labels per call (default: 10; capped at 100)
    target: "*"                  # "triggering" (default), "*", or number
    target-repo: "owner/repo"    # cross-repository
    allowed-repos: ["org/repo1", "org/repo2"]  # additional allowed repositories
