...
safe-outputs:
  assign-to-agent:
    ...
    reasoning_effort: "high"   # optional model-specific reasoning effort
...
`reasoning_effort` accepts model-specific strings and is forwarded as `agent_assignment.reasoning_effort` only when the selected agent and model support it. Unsupported or invalid values produce a warning and are omitted without failing the assignment. The field also supports expressions:
