...
safe-outputs:
  assign-to-agent:
    ...
    reasoning_effort: "high"   # optional reasoning effort
...
`reasoning_effort` accepts `none`, `minimal`, `low`, `medium`, `high`, or `xhigh` and is forwarded as `agent_assignment.reasoning_effort` without enforcing model-specific capabilities. Invalid values produce a warning and are omitted without failing the assignment. The field also supports expressions:
