...
- **Lockdown Mode**: Required when `lockdown: true` is set in workflow frontmatter. The workflow will fail at runtime if the token is not configured.
...
Lockdown mode is only automatically enabled for public repositories when `GH_AW_GITHUB_TOKEN` is configured:

- When `lockdown: true` is set explicitly, `GH_AW_GITHUB_TOKEN` **must** be configured or the workflow fails
- For public repositories, automatic lockdown requires both:
  1. `GH_AW_GITHUB_TOKEN` configured as a repository secret
  2. `lockdown: true` set in the workflow frontmatter
- Without `GH_AW_GITHUB_TOKEN`, workflows in public repositories run without lockdown mode
- See [Lockdown Mode documentation](/gh-aw/reference/lockdown-mode/) for security considerations
...
**Example Workflow with Lockdown**:

```yaml wrap
---
lockdown: true  # Requires GH_AW_GITHUB_TOKEN
engine: copilot
tools:
  github:
    mode: remote
...
If `lockdown: true` is set but `GH_AW_GITHUB_TOKEN` is not configured, the workflow fails immediately with:

```
Lockdown mode is enabled (lockdown: true) but GH_AW_GITHUB_TOKEN is not configured.
Please configure GH_AW_GITHUB_TOKEN as a repository secret.
```
