...
For no-shell Go repository work with `create-pull-request`, use the experimental, opt-in [`tools.profile: go`](/gh-aw/reference/engines/#go-repository-tool-profile-toolsprofile). `profile` accepts a string or an array and merges across shared workflow imports. This profile suppresses the PR-induced model shell defaults, including the ordinary shell commands they would otherwise enable, while retaining editing and Git/publication infrastructure. Without the profile, the existing PR shell defaults are unchanged. The profile requires the bundled Copilot SDK driver inside AWF and explicit `bash: false` (or `[]`) and `cli-proxy: false`; its fixed `go_repository` actions are not general shell access.
