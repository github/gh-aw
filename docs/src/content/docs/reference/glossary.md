...
A reserved grader that evaluates operational repository outcomes using a repository-relative Bash evaluator frozen at compile time with its SHA-256 digest recorded for reproducibility. gh-aw invokes the evaluator once with run, event, and config JSON. It returns an ordered array of unique domain-named metrics whose values are in `[0,1]` or `null`; the first metric is primary and is also exposed as the grader's top-level value. The evaluator receives the workflow token through `GH_TOKEN` with the agent job's declared permissions, but not workflow secrets, and enabling the grader does not add evidence permissions. See [Graders Reference](/gh-aw/experimental/trace-graders/#operational-value-grader) and the `operational value designer` skill (`/operational-value-designer`).
