...
| `schedule-seed` | string | No | Repository slug in `owner/repo` form used for fuzzy schedule scattering by `gh aw compile`. The `--schedule-seed` flag takes precedence. |
