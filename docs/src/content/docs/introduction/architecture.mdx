...
description: Security architecture for GitHub Agentic Workflows across hosted runners, ARC with Docker-in-Docker, and Cloud Hypervisor microVM isolation.
sidebar:
  order: 3
...
The default agent runtime uses Docker container isolation. Eligible GitHub-hosted runners can instead use Cloud Hypervisor to place the agent in a KVM-isolated microVM. These runtimes add isolation boundaries but do not remove trust in the runner host, firewall, MCP Gateway, container runtime, kernel, hypervisor, or hardware.
...
flowchart TB
    ...
    subgraph Isolation["🛡️ Isolation Layer"]
        AWF["Agent Workflow Firewall<br/>Docker or Cloud Hypervisor Isolation"]
...
> [!IMPORTANT]
> Cloud Hypervisor is incompatible with `runner.topology: arc-dind`. The compiler rejects incompatible runtime, topology, and AWF-version combinations and verifies the required GitHub-hosted Ubuntu x86_64 KVM environment before agent startup.
...
| **Substrate** | Cloud Hypervisor KVM microVM | Agent-to-host kernel and memory access |
