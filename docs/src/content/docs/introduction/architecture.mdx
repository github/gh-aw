...
When **lockdown mode is enabled** (by setting `lockdown: true` and configuring [`GH_AW_GITHUB_TOKEN`](/gh-aw/reference/auth/#gh_aw_github_token)), the GitHub MCP server:
