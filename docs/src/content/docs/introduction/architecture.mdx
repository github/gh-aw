...
- **Prompted Analysis**: Detection uses a supported engine with the `detection` model alias by default and a security-focused system prompt that instructs the agent to identify threats
...
- **AI Detection**: Default AI-powered analysis using a supported detection engine and the `detection` model alias
