...
Lockdown mode must be explicitly enabled by setting `lockdown: true` in your workflow frontmatter and configuring [`GH_AW_GITHUB_TOKEN`](/gh-aw/reference/auth/#gh_aw_github_token) as a repository secret. When enabled, the GitHub MCP server:
