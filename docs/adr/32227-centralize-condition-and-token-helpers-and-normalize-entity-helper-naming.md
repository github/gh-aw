...
2. Token-resolution functions **MUST** use the `resolve*` prefix when their primary responsibility is to walk a precedence chain and return an AI Credit (e.g., `resolvePRCheckoutToken`, `resolveStaticCheckoutToken`, `resolveProjectToken`, `resolveProjectURLAndToken`).
