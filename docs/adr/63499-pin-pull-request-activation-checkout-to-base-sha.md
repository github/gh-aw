...
We will pin activation-job sparse checkouts to the pull request base SHA when the active event is `pull_request`. For other trigger types, including mixed-trigger workflows outside an active pull request and `workflow_call`, the compiler will preserve the existing fallback refs instead of forcing the base SHA behavior universally. We chose this because activation-time instructions and imported content are security-sensitive, and the PR evidence shows that using PR-head content at that stage can let contributors influence the system that evaluates their own changes.
