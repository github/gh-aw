...
We will apply `runner.topology: arc-dind` to the threat-detection job only when `safe-outputs.threat-detection.runs-on` is explicitly set for that job. gh-aw will otherwise treat the detection job as running on its default GitHub-hosted runner and will avoid ARC/DinD-specific path rewriting, tool-cache redirection, and AWF topology configuration there. We chose this because runner topology is a property of the runner executing the detection job, not a workflow-wide assumption that can safely be copied from the primary agent job.
...
- Regression tests now verify that ARC/DinD code generation is omitted by default and preserved only when the detection job declares its own runner.

#### Negative
- ARC/DinD behavior for detection is now conditional on `safe-outputs.threat-detection.runs-on`, which adds another coupling that maintainers must understand when debugging runner-specific behavior.
...
- Documentation now states that the detection job only inherits ARC/DinD topology when it declares its own runner override.
