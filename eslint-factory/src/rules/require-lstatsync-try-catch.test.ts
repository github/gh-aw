...
describe("require-lstatsync-try-catch", () => {
  it("valid: fs.lstatSync and fs.readlinkSync inside try block pass", () => {
    cjsRuleTester.run("require-lstatsync-try-catch", requireLstatSyncTryCatchRule, {
      valid: [
        ...
        `function f() { try { fs.lstatSync(current); } catch (e) {} }`,
        `try { fs["lstatSync"](path); } catch (e) {}`,
      ],
      invalid: [],
    });
  });
  ...
  it("valid: other fs methods not in scope are ignored", () => {
    cjsRuleTester.run("require-lstatsync-try-catch", requireLstatSyncTryCatchRule, {
      valid: [`fs.statSync(path);`, `fs.existsSync(path);`, `mockFs.lstatSync(path);`, `storage.readlinkSync(path);`],
      invalid: [],
    });
  });
  ...
  it("invalid: fs.lstatSync outside try/catch is flagged", () => {
    cjsRuleTester.run("require-lstatsync-try-catch", requireLstatSyncTryCatchRule, {
      ...
      invalid: [
        {
          code: `stat = fs.lstatSync(current);`,
          errors: [{ messageId: "requireTryCatch", data: { method: "lstatSync", arg: "current" }, suggestions: [expectedWrapInTryCatchSuggestion("lstatSync", "stat = fs.lstatSync(current);")] }],
        },
      ],
    });
  });
      ...
      invalid: [
        {
          ...
          errors: [{ messageId: "requireTryCatch", data: { method: "readlinkSync", arg: "fullPath" }, suggestions: [expectedWrapInTryCatchSuggestion("readlinkSync", "fs.readlinkSync(fullPath);")] }],
        },
      ],
    });
  });
  ...
  it("invalid: destructured fs methods outside try/catch are flagged", () => {
    cjsRuleTester.run("require-lstatsync-try-catch", requireLstatSyncTryCatchRule, {
      ...
      invalid: [
        {
          code: `const { lstatSync } = require("fs"); lstatSync(path);`,
          errors: [{ messageId: "requireTryCatch", suggestions: [expectedWrapInTryCatchSuggestion("lstatSync", "lstatSync(path);", `const { lstatSync } = require("fs"); `)] }],
        },
      ],
    });
  });
