...
describe("pr_review_buffer (factory pattern)", () => {
  ...
  describe("submitReview", () => {
    ...
    it("should append durable workflow-call-id provenance to review body when available", async () => {
      ...
      try {
        ...
        expect(callArgs.body).toContain('[gh-aw-workflow-call-id]: # "owner%2Frepo%2FCallerA"');
      } finally {
    ...
    it("should dismiss only older blocking reviews with durable workflow-call-id provenance", async () => {
      ...
      try {
        ...
        mockGithub.rest.pulls.listReviews.mockResolvedValue({
          data: [
            { id: 100, state: "CHANGES_REQUESTED", user: { login: "github-actions[bot]", type: "Bot" }, body: '[gh-aw-workflow-call-id]: # "owner%2Frepo%2FCallerA"\nOld blocking review' },
        ...
        expect(result.success).toBe(true);
        expect(mockGithub.rest.pulls.createReview.mock.calls[0][0].body).toContain('[gh-aw-workflow-call-id]: # "owner%2Frepo%2FCallerA"');
