...
describe("searchEntitiesWithExpiration", () => {
  ...
  it("should find issue #12669 with Workflow prefix and legacy expiration format", async () => {
    // Issue #12669: https://github.com/githubnext/gh-aw/issues/12669
    // This issue was not detected because it has "> Workflow:" prefix instead of "> AI generated by"
    const issue12669 = {
...
> - [x] expires  on Jan 31, 2026, 6:05 AM UTC`,
    ...
    // This issue should be found because it has an expiration marker
    // even though it uses "> Workflow:" instead of "> AI generated by"
    expect(result.items).toHaveLength(1);
    expect(result.items[0]).toEqual(issue12669);
    expect(result.stats.totalScanned).toBe(1);
    expect(global.core.info).toHaveBeenCalledWith(expect.stringMatching(/Found issue #12669 with expiration marker \(legacy format\):/));
  });
});
