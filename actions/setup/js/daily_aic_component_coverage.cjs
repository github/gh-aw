...
const KNOWN_ZERO_USAGE_EVAL_JOBS = new Set(["34708221982:103592729178:1"]);
...
function provesFailedEvalsHadNoUsage(job, runId) {
  ...
  const knownZeroUsageJob = KNOWN_ZERO_USAGE_EVAL_JOBS.has(`${runId}:${job.id}:${job.run_attempt}`);
  // This historical job predates failure-path accounting uploads. Its logs prove
  // AWF installation failed before model execution and the collector ran.
  if (knownZeroUsageJob && failed("Install AWF binary") && succeeded("Collect evals token usage")) return true;
...
function sumCoveredComponents(directory, components, artifactCreatedAt, artifacts, usageArtifactName, attempt, runId) {
  ...
  for (const [name, job] of components) {
    ...
    if (!selected && name === "evals" && provesFailedEvalsHadNoUsage(job, runId)) {
      logComponentAIC(runId, name, job, 0, "failed_before_accounting", {
        source: candidateStates[0].file,
      });
      continue;
