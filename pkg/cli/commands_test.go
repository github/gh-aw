...
// TestDownloadAgentFileFromGitHub_URLPatching tests that URLs are patched for release versions
func TestDownloadAgentFileFromGitHub_URLPatching(t *testing.T) {
...
// Download the file
content, err := downloadAgentFileFromGitHub(false)
if err != nil {
t.Skipf("Skipping test - failed to download: %v (may be network issue)", err)
}

// Check that URLs were patched to use the release version
if strings.Contains(content, "/blob/main/") {
t.Errorf("Expected URLs to be patched to use release version, but found /blob/main/ in content")
}

if !strings.Contains(content, "/blob/v1.2.3/") {
t.Errorf("Expected URLs to be patched to /blob/v1.2.3/, but not found in content")
}
}

// TestDownloadAgentFileFromGitHub_DevBuild tests that main branch is used for dev builds
func TestDownloadAgentFileFromGitHub_DevBuild(t *testing.T) {
// Save current version
originalVersion := GetVersion()
defer SetVersionInfo(originalVersion)

// Test with a dev version
SetVersionInfo("abc1234-dirty")
...
// Check that content was downloaded
if len(content) == 0 {
t.Errorf("Expected content to be downloaded, but got empty string")
}

// Content should contain the expected structure
if !strings.Contains(content, "# GitHub Agentic Workflows Agent") {
