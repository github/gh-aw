...
// InferCompatibleToolsets infers the smallest compatible subset of toolsets from the default
// toolsets based on the provided permissions. This is used when permissions are specified
	...
	for _, toolset := range DefaultGitHubToolsets {
		...
		for _, scope := range perms.ReadPermissions {
			...
			if !granted || grantedLevel == PermissionNone {
				permissionsValidationLog.Printf("Toolset %s incompatible: missing read permission %s", toolset, scope)
				isCompatible = false
				break
			}
		}
		...
		if isCompatible && !readOnly {
			for _, scope := range perms.WritePermissions {
				...
				if !granted || grantedLevel != PermissionWrite {
					permissionsValidationLog.Printf("Toolset %s incompatible: missing write permission %s", toolset, scope)
					isCompatible = false
					break
				}
			}
