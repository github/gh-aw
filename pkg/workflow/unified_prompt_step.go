...
Temporary IDs: Some safe output tools support a temporary ID field (usually named temporary_id) so you can reference newly-created items elsewhere in the SAME agent output (for example, using #aw_abc123def456 in a later body). 
...
- If you DO need cross-references/chaining, you MUST match this EXACT validation regex: /^aw_[0-9a-f]{12}$/i
- Format: aw_ prefix followed by EXACTLY 12 hexadecimal characters (0-9, a-f, case-insensitive)
- Valid hex characters ONLY: 0123456789abcdef (lowercase or uppercase)
- INVALID examples: aw_bundle000004 (contains 'u', 'n', 'l' which are NOT hex), aw_issue_001 (contains '_' and non-hex), aw_npm123 (too short)
- VALID examples: aw_abc123def456, aw_000011112222, aw_deadbeef9876, aw_1a2b3c4d5e6f
- To generate valid IDs: use exactly 12 random hex digits or omit the field to let the system auto-generate

Do NOT invent other aw_* formats — downstream steps will reject them with validation errors matching against /^aw_[0-9a-f]{12}$/i.
